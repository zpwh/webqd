...
```html
<body>
    <div id="div" ><p v-if="seen">现在你看到我了</p></div>
</body>
<script>
    var app = new Vue({
        el:'#div',
...
- 对象语法
  ...
  ```html
  <div id="app" v-bind:class="{active: isActive}">hei</div>
  <script>
      var vm = new Vue({
          el: '#app',
          data: {
...
- 数组语法
  ...
  ```html
  <div id="app" v-bind:class="[activeClass, dangerClass]">hei</div>
  <script>
      var vm = new Vue({
          el: '#app',
          data: {
...
- 对象语法
  ...
  ```html
  <div id="app" v-bind:style="{color: redColor, fontSize: font + 'px'}">hei</div>
  <script>
      var vm = new Vue({
          el: '#app',
          data: {
...
```html
<div id="div"><input type="text" :value="input_val"></div>
<script>
    var app = new Vue({
        el: '#div',
        data: {
...
```html
<div id="div"><input type="text" v-model="input_val" ></div>
<script>
    var app = new Vue({
        el: '#div',
        data: {
...
通过 v-model 指令展示表单数据，vue实例对象中的数据改变 会影响dom中的数据,dom中数据改变,同时会影响vue实例中的数据,此时就完成了 ***双向数据绑定***  ；
...
<div id="div">
    ...
    <input type="text" v-model.trim='input_val'> <br>
    <!-- 修饰符可以使用关联式操作使用 -->
    <input type="text" v-model.number.trim.lazy='input_val'> <br>
    {{input_val}}
</div>
...
鼠标事件 : click mousemover mousedwon mouseup  mouselive mouseout mouseover dbclick 
...
```html
<div id="app"><input type="button" value="按钮" v-on:click="cli"></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {
...
```html
<div id="app"><a href="http://www.qq.com" id="a">腾百万</a></div>
<script>
...
```html
<div id="app"><a href="http://www.qq.com" @click.prevent="cli">腾百万</a></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {},
...
```html
<div id="app"><a href="http://www.qq.com" @click.once="cli($event)">腾百万</a></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {},
...
```html
<div id="app"><input type="text"  @keyup.enter="keyup"></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {},
...
```html
<div id="app"><input type="button" value="按钮" @click.shift="cli"></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {},
...
```html
<div id="app"><input type="button" value="按钮" @click.middle="cli"></div>
<script>
    var vm = new Vue({
        el: '#app',
        data: {},
...
```html
<div id="app"><p v-show="is_show">Vue</p></div>
<!-- v-show 实际是使用css display控制显示和隐藏 -->
<script>
    var vm = new Vue({
        el:'#app',
        data:{
...
```html
<div id="app">
    <!-- vue 指令中的if  esle 判断的基本逻辑和普通的 if else 逻辑一致 -->
...
```html
<div id="app">
    <ul><li v-for="(val,key) in arr">{{val}}---{{key}}</li></ul>
    <ul><li v-for="(val,key) in obj">{{val}}---{{key}}</li></ul>
</div>
<script>
    var vm = new Vue({
        el: '#app',
...
<div id="app"><p v-cloak>{{obj.id}}</p></div>
<script src="./vue.js"></script>
<script>
    setTimeout(() => {
        var vm = new Vue({
...
```html
<div id="app">
    <!-- 数据只绑定一次,单次绑定,后面数据发生变化, msg 不会受到影响-->
    <p v-once>{{msg}}</p>
</div>
<script>
    var vm = new Vue({
...
<div id="div">
    ...
    <!-- 一百次调用 -->
    {{fulln}}
    <!-- 使用计算属性时,属性的名字和computed中的方法名保持一致 -->
    <!-- 如果加上括号会报错 -->
    <!-- 因为内部获取的值就是一个fullName的属性值 -->
    <!-- {{fullName()}} -->
</div>
<script>
    var app = new Vue({
...
computed: {
    // 未完成任务个数
    // 不会每次调用。当 xing 或者ming  变化时才会调用。调用时,会将函数执行的结果放入一个和方法(函数) 同名的属性中。以供模板使用,所以模板中不能加();
    getNu() {
    ...
    var app = new Vue({
        ...
        methods: {
            click: function () {
                ...
                // 使用vue ref 属性获取dom数据
                // 需要在事件处理器中获取 P 元素节点对象
                // vue 提供$refs的属性,让我们可以获取到 节点对象
                // this.$refs.pv.innerHTML = 1;
...
<style>
    ...
    .man-enter-active,.man-leave-active {transition: opacity 4s;}
    .man-enter,.man-leave-to {opacity: 0;}
</style>
<div id="app">
    <button @click="go">显示/隐藏</button>
